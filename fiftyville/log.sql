...
SELECT activity, license_plate, minute FROM bakery_security_logs WHERE year = 2021 AND month = 7 AND day = 28 AND hour = 10 AND minute > 15 AND minute <25;
...
--Right now, we are down to 5 suspects: Iman, Luca, Diana, Bruce
...
--At this time, we have narrowed it down to Diana, Bruce and the suspected accomplices Jaqueline and Robin respectively

--Checking which is the first flight away, and where does it go
SELECT city FROM airports JOIN flights ON airports.id = flights.destination_airport_id WHERE origin_airport_id IN (SELECT id FROM airports WHERE city = "Fiftyville") AND year = 2021 AND month = 7 AND day = 29 ORDER BY hour, minute LIMIT 1;

--We now know he/she went to New York city

--Finding flight id
SELECT id FROM flights WHERE origin_airport_id IN (SELECT id FROM airports WHERE city = "Fiftyville") AND destination_airport_id IN (SELECT id FROM airports WHERE city = "New York City") AND year = 2021 AND month = 7 AND day = 29 ORDER BY hour, minute LIMIT 1;
